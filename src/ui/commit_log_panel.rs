...
pub fn render(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    let block = theme::modal_elevated_block("Commits");
...
fn render_log_list(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    if repo.log_entries.is_empty() {
        let empty = Paragraph::new("No commits found.").style(theme::modal_muted_style());
        frame.render_widget(empty, area);
        return;
    }
    ...
    for entry in &repo.log_entries {
        items.push(ListItem::new(Line::from(vec![
            Span::styled(format!("{} ", entry.hash), theme::modal_accent_style()),
            Span::styled(&entry.subject, theme::modal_text_style()),
        ])));
    }
...
fn render_commit_detail(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    let mut lines = vec![
        Line::from(vec![
            Span::styled("Hash:    ", theme::modal_muted_style()),
            Span::styled(&entry.hash, theme::modal_accent_style()),
        ]),
        Line::from(vec![
            Span::styled("Author:  ", theme::modal_muted_style()),
            Span::styled(&entry.author, theme::modal_text_style()),
        ]),
        Line::from(vec![
            Span::styled("Date:    ", theme::modal_muted_style()),
            Span::styled(&entry.date, theme::modal_text_style()),
        ]),
        Line::default(),
        Line::from(Span::styled(
            &entry.subject,
            theme::modal_text_style().add_modifier(Modifier::BOLD),
        )),
    ];
    ...
    if !body.is_empty() {
        ...
        for line in body.lines() {
            lines.push(Line::from(Span::styled(line, theme::modal_text_style())));
        }
    }
    ...
    let separator = ratatui::widgets::Block::default()
        .borders(ratatui::widgets::Borders::LEFT)
        .border_style(theme::separator_style())
        .padding(ratatui::widgets::Padding::left(1));
    let detail_area = separator.inner(area);
